...
кнопку, которая по нажатию будет выводить что-то в чат. И в принципе разработчик ограничен лишь своей
...
или объект класса, наследующегося от VisualObject (виджет). Таким образом, игра сапер представляет из себя одно из
...
от VisualObject и лишь добавляет некоторые поля и функционал (например, функции всплывающего окна ShowPopUp, Alert, Confirm).
...
Корень дерева доступен по свойству Root (типа RootVisualObject). Учтите, что получить доступ к этому полю
...
и загружаются на хуке GamePostInitialize, таким образом в вашем плагине вам достаточно создать
свой интерфейс и добавить в TUI.
...
Этот метод создает объект RootVisualObject, (который, в свою очередь,
наследуется от VisualObject), затем добавляет его в список корней TUI.
...
Этот метод позвоялет автоматически располагать детей, добавленных с помощью метода AddToLayout,
...
```
<details><summary> <sub><b><ins>Параметры</ins></b> (нажмите сюда, чтобы развернуть)</sub> </summary>
<p>
...
Пример:
```cs
// Настраиваем конфигурацию сетки grid. Указываем, что нужно все ячейки заполнить автоматически.
...
с помощью предмета The Grand Design (Великий план). Каждому нажатию ставится в соответствие
...
* bool **Red**, **Green**, **Blue**, **Yellow**, **Actuator**, **Cutter**
	* Включен ли красный/зеленый/желтый провод/активатор/резак. Актуально только
...
принимающее значение TouchState.Begin в первое нажатие из промежутка,
значение TouchState.End в последнее нажатие из промежутка
...
Функция Invoke по умолчанию вызывает функцию-колбэк, хранящуюся в поле VisualObject.Callback.
...
При этом гарантируется, что загрузка произойдет после события GamePostInitialize.
...
FullName - полное название в дереве интерфейса, например:
...
элементов.
Если мы контейнеру установим свойству Name значение "lol", то то же самое
...
void DBWrite()
```
Эти функции по умолчанию пытаются вызывать функции Configuration.Custom.DBRead и Configuration.Custom.DBWrite.
...
Для того, чтобы заставить объект рисоваться с помощью отправок секций, достаточно установить его полю ForceSection значение true.
...
* object **provider**
	* Объект провайдера тайлов (блоков), значение по умолчанию - null (интерфейс будет
...
Потяните за задний фон вниз - layout поедет вниз. И наоборот.
